...
2. ライブラリ "PinChangeInterrupt” をPCに[インストール](#ライブラリ-pinchangeinterrupt-のインストール)する。
...
8. ファームウエアをRaduinoボードに[アップロード（書込む）](#スケッチのアップロード書込み)する。
...
IDEが起動して、メニューが日本語になっていない場合には次の手順で日本語に変更します。
通常はPCの言語が自動的に適用されるようになっていますので、日本語のPCであれば日本語のメニューが表示されます。
...
フォルダ "bitx40-master" の中から、ファイル "raduino_v1.27.7.ino" を探してダブルクリックします。
