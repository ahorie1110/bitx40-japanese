...
- 送信中は周波数のチューニングを無効にする（送信時のフラッタや“FM-ing”を防ぐため。これには、ピンA0（黒線）に“PTT SENSE”線が接続されている必要があります(配線手順は　　を参照)。

v1.09
- RIT（SPLIT）機能を追加した。これには“PTT　SENSE”線がピンA0（黒線）に接続されている必要がある(配線手順は　を参照)。
